...
async def _generate_output_with_llm(
    ...
    """Use OpenAI to generate a contextual mock output for this task."""
    from app.config import settings

    if not settings.OPENAI_API_KEY:
        return f"**Task completed:** {prompt}\n\n> Mock output (no API key configured)"
    ...
    try:
        ...
        type_instructions = {
            ...
            "code_execution": "You executed code. Show the code in a fenced code block, then show the results including any computed values. Use tables for structured results.",
            "report": "You wrote a report. Write a proper multi-section markdown document with headings, findings, and conclusions. Make it 200-400 words.",
...
- If you have dependency inputs, reference and build on them"""
        ...
        return response.choices[0].message.content or f"**Completed:** {prompt}"
...
class MockAgent(BaseAgent):
    ...
    async def execute(
        ...
        summary = await _generate_output_with_llm(
            prompt, self.agent_type, dependency_outputs
        )
